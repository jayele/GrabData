...
for page in *.shtml; do
    cat $page | grep 'class="su_con"' -A 2 > /tmp/$$.58tmp
    h1=`cat $page | grep '<h1>' | awk -F '>' '{print $2}' | awk -F '<' '{print $1}'`
    name=`cat /tmp/$$.58tmp | grep '</a>' | awk -F '>' '{print $2}' | awk -F '<' '{print $1}'`
    tel=`tail -2 /tmp/$$.58tmp | head -1 | egrep -o '[0-9,-]+'`
    
    [[ -z $tel ]] && continue > /dev/null
    echo $tel | grep '^400' > /dev/null #400开头的电话过滤掉
	[ $? -eq 0 ] && continue
	echo $tel | egrep '[0-9]{6,}' > /dev/null
	[ $? -ne 0 ] && continue
